...
import (
	...
	"mfinder/backend/context"
	"mfinder/backend/database"
	"mfinder/backend/database/models"
	"mfinder/backend/database/repository"
	"mfinder/backend/service/model/icp"
	service2 "mfinder/backend/service/service"
...
func mergeWithStatusContext(statCtx *context.StatusContext) (ctx.Context, ctx.CancelFunc) {
	baseCtx, cancel := ctx.WithCancel(ctx.Background())
	if statCtx == nil {
		return baseCtx, cancel
	}
	go func() {
		select {
		case <-statCtx.Done():
		case <-statCtx.Stop():
		case <-statCtx.Pausing():
		case <-statCtx.Paused():
		case <-statCtx.Error():
		}
		cancel()
...
func (r *Bridge) query(pageNum, pageSize int, serviceType string, query string, authErrorRetryNum, forbiddenErrorRetryNum uint64) (*Result, error) {
	ctx2, cancel := mergeWithStatusContext(nil)
	defer cancel()
	var err1 error
...
func (r *Bridge) queryWithStatusContext(pageNum, pageSize int, serviceType string, query string, authErrorRetryNum, forbiddenErrorRetryNum uint64, statCtx *context.StatusContext) (*Result, error) {
	ctx2, cancel := mergeWithStatusContext(statCtx)
	defer cancel()
	var err1 error
	var canceled bool
	result, _ := backoff.RetryWithData(func() (*Result, error) {
...
func (r *Bridge) taskInitState(taskID int64, resetTask bool) (*TaskState, error) {
	...
	bufferSize := int(r.app.Config.ICP.Concurrency) * 2
	if bufferSize < 1 {
		bufferSize = 1
	}
	t := &TaskState{
		...
		startTime:     time.Now(),
		resultChan:    make(chan *models.ICPTaskSlice, bufferSize),
	}
