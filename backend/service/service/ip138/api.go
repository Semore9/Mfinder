...
func setCommonHeaders(req *http.Request, referer string) {
	req.Header.Set("User-Agent", Ip138UserAgent)
	if referer != "" {
		req.Header.Set("Referer", referer)
	}
	if strings.HasSuffix(req.URL.Path, ".do") {
...
func (d *domain) GetCurrentIP(domain string) ([]*IPItem, string, error) {
	...
	if len(ips) == 0 {
		...
		code, err2 := d.client.Domain.write(tokenHint)
		if err2 != nil {
			return nil, "", err2
		}
		if code == 21001 {
...
func (d *domain) read() (map[string]string, *Response, error) {
	...
	setCommonHeaders(request, fmt.Sprintf("%s%s/", Ip138BaseURL, d.client.domain))
	response, err := d.client.http.Do(request)
	if err != nil {
		return nil, nil, err
	}
	defer response.Body.Close()
	if response.StatusCode == 502 {
		return nil, nil, errors.New("可能被ban了")
	}
	body, err := io.ReadAll(response.Body)
	if err != nil {
		return nil, nil, err
...
// 21001 token和域名不匹配；
// 21002 禁止查询该域名
func (d *domain) write(tokenHint string) (int, error) {
	token := tokenHint
	if token == "" {
		var err error
...
func (d *domain) getLocation(ip, sign string) (string, error) {
	...
	setCommonHeaders(request, "https://site.ip138.com/")
	response, err := d.client.http.Do(request)
	if err != nil {
		return "", err
	}
