...
import (
	...
	"mfinder/backend/service/model/wechat"
...
// DetectAppletPath 检测微信小程序路径
func (r *Bridge) DetectAppletPath() string {
	return r.app.DetectWeChatAppletPath()
}

// AutoSetAppletPath 自动检测并设置微信小程序路径
func (r *Bridge) AutoSetAppletPath() error {
	detectedPath := r.app.DetectWeChatAppletPath()
	if detectedPath == "" {
		return errors.New("未检测到有效的微信小程序路径")
	}
	
	return r.SetAppletPath(detectedPath)
}
