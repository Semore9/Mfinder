...
var DefaultRegex = matcher.RuleList{
	// 仅适用于前端
	// TODO 如果值包含转义双引号，则需要修改
	{
		ID:     atomic.AddInt64(&ruleID, 1),
		Name:   "Path",
		Enable: true,
		Regexes: []string{
			// 相对路径URL,包含可执行文件后缀
	...
	},
	{
		...
		Regexes: []string{
			...
			// UCloud：public_key / private_key
			// 百度云：Access Key ID / Secret Access Key
			// 亚马逊AWS：Access Key ID / Secret Access Key
			// 微软Azure：Subscription ID / Tenant ID / Client ID / Client Secret
			// 华为云：Access Key ID  / Secret Access Key
			// 京东云：Access Key ID / Access Key Secret
			// 青云：qy_access_key_id / qy_secret_access_key
			// 金山云：Access Key ID / Secret Access Key
	...
	},
	{
		...
		Enable: true,
		Regexes: []string{
			"(?i)[\"']?(?:mch[_-]?id|mchid|merchant[_-]?id)[\"']?[^\\S\r\n]*[=:][^\\S\r\n]*('[^'\\r\n]+'|\"[^\"\\r\n]+\"|[0-9]{8,}\b)",
			"(?i)[\"']?(?:api(?:_v3)?[_-]?key|pay[_-]?key|partner[_-]?key)[\"']?[^\\S\r\n]*[=:][^\\S\r\n]*('[^'\\r\n]+'|\"[^\"\\r\n]+\"|[A-Za-z0-9]{16,})",
			"(?i)[\"']?serial[_-]?no[\"']?[^\\S\r\n]*[=:][^\\S\r\n]*('[^'\\r\n]+'|\"[^\"\\r\n]+\"|[A-Fa-f0-9]{12,})",
			"(?i)[\"']?apiclient_(?:cert|key)[\"']?[^\\S\r\n]*[=:][^\\S\r\n]*('[^'\\r\n]+'|\"[^\"\\r\n]+\"|[\\w./\\\\-]+\\.pem\\b)",
		},
	},
	{
		ID:     atomic.AddInt64(&ruleID, 1),
		...
		Regexes: []string{
			"(?i)\\bwx[0-9a-f]{16}\\b",
			"(?i)[\"']?(?:app|component|authorizer)[_-]?secret[\"']?[^\\S\r\n]*[=:][^\\S\r\n]*('[^'\\r\n]+'|\"[^\"\\r\n]+\"|[A-Fa-f0-9]{16,})",
			"(?i)[\"']?(?:component|authorizer)[_-]?(?:appid|token|refresh[_-]?token)[\"']?[^\\S\r\n]*[=:][^\\S\r\n]*('[^'\\r\n]+'|\"[^\"\\r\n]+\"|[A-Za-z0-9_-]{16,})",
		},
	},
	{
		ID:     atomic.AddInt64(&ruleID, 1),
		...
		Enable: true,
		Regexes: []string{
			"(?i)[\"']?(?:session[_-]?key|session[_-]?token)[\"']?[^\\S\r\n]*[=:][^\\S\r\n]*('[^'\\r\n]+'|\"[^\"\\r\n]+\"|[A-Za-z0-9+/=]{16,})",
			"(?i)[\"']?(?:openid|unionid)[\"']?[^\\S\r\n]*[=:][^\\S\r\n]*('[^'\\r\n]+'|\"[^\"\\r\n]+\"|[A-Za-z0-9_-]{10,})",
		},
	},
	{
		...
		Enable: true,
		Regexes: []string{
			"(?i)[\"']?(?:tencentcloud|qcloud|cos|scf)[_-]?(?:secret[_-]?id|secret[_-]?key)[\"']?[^\\S\r\n]*[=:][^\\S\r\n]*('[^'\\r\n]+'|\"[^\"\\r\n]+\"|[A-Za-z0-9]{16,})",
			"(?i)[\"']?(?:env[_-]?id|envid)[\"']?[^\\S\r\n]*[=:][^\\S\r\n]*('[^'\\r\n]+'|\"[^\"\\r\n]+\"|[A-Za-z0-9-]{5,})",
			"(?i)cloud://[a-z0-9-]+(?:\\.[a-z0-9-]+)+/[\\w./-]+",
		},
	},
	{
		ID:     atomic.AddInt64(&ruleID, 1),
		...
		Enable: true,
		Regexes: []string{
			"-----BEGIN [^-]{1,40}KEY-----[\\s\\S]{100,}?-----END [^-]{1,40}KEY-----",
			"-----BEGIN CERTIFICATE-----[\\s\\S]{100,}?-----END CERTIFICATE-----",
		},
	},
	{
		ID:      atomic.AddInt64(&ruleID, 1),
		Name:    "手机号",
		Enable:  true,
		Regexes: []string{"\\b(?:\\+?86)?1[3-9]\\d{9}\\b"}, // 基本格式
	},
	{
		ID:     atomic.AddInt64(&ruleID, 1),
		Name:   "身份证",
		Enable: true,
		Regexes: []string{
			"\\b[1-9]\\d{5}(?:18|19|20)\\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\\d|3[01])\\d{3}[\\dXx]\\b", // 18位身份证，前面可以有引号或空格
			"\\b[1-9]\\d{5}\\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\\d|3[01])\\d{3}\\b",                    // 15位身份证，前面可以有引号或空格
		},
	},
	{
		ID:     atomic.AddInt64(&ruleID, 1),
		...
		Regexes: []string{
			...
			"(?i)[\\[\"']?(db|database)[_-]?(username|user)[\"'\\]]?[^\\S\r\n]*[=:][^\\S\r\n]*('[^']+'|\"[^\"]+\"|[\\w-.]+\\b)",
			"(?i)[\\[\"']?(db|database)[_-]?password[\"'\\]]?[^\\S\r\n]*[=:][^\\S\r\n]*('[^']+'|\"[^\"]+\"|[\\w-.]+\\b)",
			"(?i)[\\[\"']?(db|database)[_-]?host[\"'\\]]?[^\\S\r\n]*[=:][^\\S\r\n]*('[^']+'|\"[^\"]+\"|[\\w-.]+\\b)",
			"(?i)[\\[\"']?(db|database)[_-]?port[\"'\\]]?[^\\S\r\n]*[=:][^\\S\r\n]*('[^']+'|\"[^\"]+\"|[\\w-.]+\\b)",
			"(?i)[\\[\"']?(db|database)[_-]?(name|database)[\"'\\]]?[^\\S\r\n]*[=:][^\\S\r\n]*('[^']+'|\"[^\"]+\"|[\\w-.]+\\b)",
			"(?i)[\\[\"']?(db|database)[_-]?url[\"'\\]]?[^\\S\r\n]*[=:][^\\S\r\n]*('[^']+'|\"[^\"]+\"|[\\w-:/.]+\\b)",
			"(?i)[\\[\"']?(db|database)[_-]?driver[\"'\\]]?[^\\S\r\n]*[=:][^\\S\r\n]*('[^']+'|\"[^\"]+\"|[\\w-.]+\\b)",
			"(?i)[\\[\"']?(db|database)[_-]?type[\"'\\]]?[^\\S\r\n]*[=:][^\\S\r\n]*('[^']+'|\"[^\"]+\"|[\\w-.]+\\b)",
			"(?i)(jdbc:)?(mysql|postgresql|oracle|sqlserver|sqlite|mariadb)://[\\w-.]+(?::\\d+)?/[\\w-]+(?:\\?[\\w-%&=]*)?",
		},
	},
	{
		ID:     atomic.AddInt64(&ruleID, 1),
		...
		Enable: true,
		Regexes: []string{
			"(?i)(?:http[s]?://)([\\w-]+\\.)+[\\w-]+(/[\\w-./?%&=]*?)\\.(jpg|jpeg|png|gif|bmp|webp|svg|ico)",
			"(?i)[\"'](/[\\w-]+(?:/[\\w-./?%&=]*?))\\.(jpg|jpeg|png|gif|bmp|webp|svg|ico)[\"']",
		},
	},
}

// NucleiRegex 暂没用上，太多提取很慢
var NucleiRegex = []string{
