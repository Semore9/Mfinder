...
const IcpTask = React.forwardRef<IcpTaskProps, any>((props, ref) => {
    ...
    useEffect(() => {
        ...
        const off = EventsOn(event.ICPBatchQueryStatusUpdate, (data: EventDetail) => {
    ...
    const updateCell = useCallback((data: icp.Task) => {
        ...
        const rowNode = api.getRowNode(String(data.taskID))
        if (rowNode) {
    ...
    const header = (
        <IcpHeader
            ...
            />}
            actions={<>
                <Button type="primary" size={"small"} onClick={() => setOpen(true)}>新建任务</Button>
                <Button size={"small"} onClick={() => refresh()}>刷新</Button>
            </>}
            description="批量查询：支持历史记录联想，每行一个目标的新建任务可快速粘贴多条记录"
        />
    )
    ...
    return (<Flex vertical style={{width: "100%", height: "100%", paddingBottom: '5px', boxSizing: 'border-box'}} gap={12}>
        <Modal
            open={open}
            destroyOnClose
            closable={false}
        ...
        <Flex vertical style={{width: "100%", height: "100%"}} gap={12}>
            {header}
...
const IcpTaskResult = forwardRef<IcpTaskResultRef>((props, ref)=>{
    ...
    const header = taskID !== 0 ? (
        <IcpHeader
            ...
            />}
            description="任务结果：可继续按名称过滤，导出将获取当前筛选结果"
        />
    ) : null
    ...
    return <Flex vertical style={{height: "100%", paddingTop: '10px', boxSizing: 'border-box', gap: '12px'}}>
        {header}
        <div style={{width: "100%", height: "100%"}}>
            <AgGridReact
                {...AGGridCommonOptions}
