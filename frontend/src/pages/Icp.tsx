...
import {Button, Flex, Modal, Pagination, Radio, Select, Tag, Tooltip,} from 'antd';
...
import {IcpFooterBar, IcpHeader} from "@/component/IcpLayout";
    ...
    const header = (
        <IcpHeader
            candidate={<Candidate<string>
                ...
                />}
            />}
            description="ICP 备案查询：请输入单位名称或域名或备案号，请勿使用子域名或带 http:// 等前缀"
            align="center"
        />
    )

    return (<Flex vertical style={{width: "100%", height: "100%"}} gap={12}>
        {header}
    ...
    return <>
        ...
        >
            <Flex vertical gap={10}>
                ...
                <Number labelWidth={160} label={"批量查询代理超时（ns）"} width={200} value={cfg.ICP.Timeout}
        ...
        <Flex vertical style={{height: '100%', width: '100%'}}>
            <Flex>
                ...
                <Radio.Group
                    ...
                    options={[
                        {
                            label: <Flex align={"center"}>单个查询<Tooltip placement={"bottom"}
                                                                           title={"如同网页查询"}><QuestionOutlined
                                style={{fontSize: 12, color: "blue"}}
                            /></Tooltip></Flex>, value: 1
                        },
                        {
                            label: <Flex align={"center"}>批量查询（Beta）<Tooltip placement={"bottom"}
                                                                                 title={"会直接导出全部结果，务必使用代理池工具，如：https://github.com/thinkoaa/Deadpool，如任务报错可点击 继续 继续任务"}><QuestionOutlined
                                style={{fontSize: 12, color: "blue"}}
