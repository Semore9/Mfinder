  ...
  return (
    <Flex justify={"left"}>
      ...
      <Input.Password
        ...
        style={{ width: props.width || "600px", marginRight: "10px" }}
        value={value}
        readOnly={!editable}
        onChange={(e) => {
      ...
      <Flex gap={10}>
        {!editable ? (
          <Button {...buttonProps} onClick={() => setEditable(true)}>
            修改
          </Button>
        ) : (
          <Flex gap={10}>
            <Button {...buttonProps} htmlType="submit" onClick={onClick}>
              保存
            </Button>
            <Button {...buttonProps} htmlType="submit" onClick={cancel}>
              取消
            </Button>
          </Flex>
        )}
